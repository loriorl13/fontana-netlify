// === PONTE NETLIFY -> ESP32 ===
// Da localhost/LAN: richieste reali dirette all'ESP32 (192.168.1.164)
// Da Netlify: richieste tramite il dominio pubblico (fontana.fontanabyloriorl.it)
const ESP_URL = 'https://fontana.fontanabyloriorl.it';
const LOCAL_ESP_URL = 'http://192.168.1.164';
...
// URL effettivo da usare: LAN in locale, dominio pubblico da Netlify
const ACTIVE_ESP_URL = isLocal ? LOCAL_ESP_URL : ESP_URL;
...
window.fetch = function(...args) {
  // Reindirizza le chiamate relative all'URL corretto dell'ESP32
  if (typeof args[0] === 'string' && args[0].startsWith('/')) {
    args[0] = ACTIVE_ESP_URL + args[0];
  }
...
function disattivaModalitaAutomatica() {
  ...
  fetch("/auto?enable=0");
    ...
    function checkWindBeforeShow(showNumber) {
      ...
      if (speedEl && modalEl) {
        const val = window.currentWindSpeedVal.toFixed(1);
        speedEl.textContent = val + ' km/h';
    ...
    // Funzione che esegue e mantiene sincronizzato lo show
    function executeShow(showNumber, elapsed = 0) {
      // 1. Avvia la grafica se non è già attiva per questo show
      if (lastActiveShowId !== showNumber || !isExecutingShow) {
        console.log(`[Show Engine] Avvio Grafica Show ${showNumber} (elapsed: ${elapsed}ms)`);
        chiudiSimulazione();
        try { avviaSimulazione(showNumber); mostraIntro(showNumber); } catch(e) {}
        isExecutingShow = true;
        lastActiveShowId = showNumber;
        
        // Reset vecchi timeout
      ...
      // 2. Gestione Audio Sincronizzato
      if (showNumber >= 1 && showNumber <= 30) {
        let audio = document.getElementById('audio' + showNumber);
        if (audio) {
            let targetTime = elapsed / 1000.0;
            const drift = Math.abs(audio.currentTime - targetTime);

            // A. Audio in pausa o fortemente desincronizzato -> Hard Seek & Play
            if (audio.paused || drift > 2.5) {
                console.log(`[Show Engine] Hard Sync Audio: drift=${Math.round(drift*1000)}ms, target=${targetTime}s`);
                ...
                const playSyncedAudio = () => {
                    ...
                    audio.play().then(() => {
                        ...
                        if (syncEl) {
                           syncEl.innerHTML = "✅ <span style='color:#00e676;'>Sincronizzato</span>";
                           setTimeout(() => { if (syncEl) syncEl.innerHTML = ""; }, 3000);
                        }
                    }).catch(e => {
                        console.warn("[Show Engine] Autoplay bloccato:", e);
                        let syncEl = document.getElementById("showSyncStatus");
                        if (syncEl) syncEl.innerHTML = "<button onclick='unlockAudio()' style='padding:5px 10px;background:#ff3b3b;color:white;border:none;border-radius:5px;'>🔊 Tocca per sbloccare l'audio</button>";
                    });
                };

                // Attendi i metadati prima di fare seek, per evitare eccezioni
                if (audio.readyState >= 1) {
                    playSyncedAudio();
                } else {
                    audio.addEventListener('loadedmetadata', playSyncedAudio, { once: true });
                    audio.load();
                }
            } 
            // B. Leggermente fuori sincrono -> Soft Seek (senza fermare/riavviare)
            else if (drift > 0.8) {
                console.log(`[Show Engine] Soft Correzione Audio: drift=${Math.round(drift*1000)}ms`);
                audio.currentTime = targetTime;
            }
        }
...
let cancelShowCounter = 0;

// Funzione chiamata dai bottoni UI (invia solo la richiesta)
async function requestShow(showNumber) {
  if (isMaintenanceActive) return;
  console.log('>>> requestShow INVIATO AL SERVER:', showNumber);
  
  let syncEl = document.getElementById("showSyncStatus");
  if (syncEl) syncEl.innerHTML = "⏳ <span>In attesa della fontana...</span>";

  // Invia il comando al server. Sarà il SyncManager a far partire grafica e audio
  // appena riceverà la conferma da /sync-status (Single Source of Truth)
  fetchWithTimeout('/show' + showNumber)
    ...
    .catch(e => {
      ...
      showCommandFeedback(true);
      if (syncEl) syncEl.innerHTML = "⚠️ <span style='color:#ff5252;'>Errore di comunicazione</span>";
      setTimeout(() => { if (syncEl) syncEl.innerHTML = ""; }, 3000);
    });
}

// Funzione chiamata dal bottone Stop (invia solo la richiesta)
function stopAllShows() {
  console.log("stopAllShows() -> Richiesta inviata al server");
  send('/stop');
}

// Funzione di arresto locale (chiamata solo dal SyncManager o in Emergenza)
function stopAllShowsLocal() {
  console.log("stopAllShowsLocal() -> Esecuzione stop UI locale");
  cancelShowCounter++; // Invalida i caricamenti audio in attesa
  chiudiSimulazione();
  ...
  for(let i=1; i<=6; i++) {
    ...
    function emergencyStop() {
      cancelShowCounter++; // Invalida qualsiasi caricamento audio in sospeso
      ...
      // Ritardiamo leggermente la seconda richiesta per non causare cadute di connessione su ESP32 e relativi falsi errori
      send('/stop');
      setTimeout(() => fetch("/auto?enable=0").catch(e => console.log(e)), 400); 
...
const SyncManager = {
  ...
  // Impostazioni polling (riduce il carico sull'ESP32)
  baseInterval: 3000,
  activeInterval: 1000,
  ...
  createIndicator() {
    this.indicator = document.getElementById('wsStatusIndicator');
    if (!this.indicator) {
      this.indicator = document.createElement('div');
      this.indicator.id = 'wsStatusIndicator';
      this.indicator.style.cssText = 'position:fixed;bottom:10px;left:10px;width:12px;height:12px;border-radius:50%;background:orange;z-index:999999;opacity:0.6;box-shadow:0 0 5px rgba(0,0,0,0.3);transition:background 0.4s;';
      this.indicator.title = 'Inizializzazione Sync...';
  ...
  async poll() {
    ...
    try {
      ...
      const tid = setTimeout(() => controller.abort(), 2000); // timeout breve per il polling
      const res = await fetch('/sync-status', { signal: controller.signal });
      clearTimeout(tid);

      if (res.ok) {
        if (!this.isOnline) this.setOnline();
        const data = await res.json();
        data._roundtrip = Date.now() - fetchStart;
        
        if (this.indicator) {
          this.indicator.style.backgroundColor = '#00e676';
          this.indicator.title = `Sync HTTP OK (RTT: ${data._roundtrip}ms)`;
        }
    ...
    // Rischedula il prossimo poll
    let nextDelay = this.baseInterval;
    if (!this.isOnline) nextDelay = this.offlineInterval;
    else if (this.currentShow > 0) nextDelay = this.activeInterval;

    if (this.syncIntervalId) clearTimeout(this.syncIntervalId);
    this.syncIntervalId = setTimeout(() => this.poll(), nextDelay);
  },

  setOnline() {
    console.log('[SyncManager] ESP32 Online');
    this.isOnline = true;
    this.offlineSince = null;
    this.hideOfflineBanner();
    // Nascondi anche vecchio banner se esiste
    if (typeof _hideEsp32OfflineBanner === 'function') _hideEsp32OfflineBanner(); 
    
    // Ricarica parametri base
    try { pollAutoSettings(); } catch(e) {}
    try { if (typeof fetchWeatherData === 'function') fetchWeatherData(); } catch(e) {}
  ...
  setOffline() {
    if (this.isOnline) {
      ...
      console.warn('[SyncManager] ESP32 Offline');
      this.showOfflineBanner();
    }
    if (this.indicator) {
      this.indicator.style.backgroundColor = 'red';
      this.indicator.title = 'Sincronizzazione Offline';
    }
    if (this.offlineBanner && this.offlineSince) {
      const secs = Math.round((Date.now() - this.offlineSince) / 1000);
      const timerEl = document.getElementById('esp32BannerTimerNew');
      if (timerEl) timerEl.textContent = `Offline da ${secs}s`;
    }
  },
  ...
  showOfflineBanner() {
     ...
     this.offlineBanner.id = 'esp32OfflineBannerNew';
     this.offlineBanner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999999;background:linear-gradient(90deg,#c62828,#e53935);color:white;font-family:sans-serif;font-size:0.85rem;font-weight:600;letter-spacing:0.5px;padding:10px 20px;display:flex;align-items:center;gap:10px;box-shadow:0 2px 12px rgba(0,0,0,0.4);animation:esp32BannerIn 0.3s ease;';
     this.offlineBanner.innerHTML = `<span style="font-size:1.2rem">⚠️</span><span id="esp32BannerTextNew">Centrale Non Raggiungibile</span><span id="esp32BannerTimerNew" style="margin-left:auto;opacity:0.7;font-size:0.75rem"></span>`;
     if (!document.getElementById('esp32BannerStyle')) {
       const s = document.createElement('style');
       s.id = 'esp32BannerStyle';
       s.textContent = '@keyframes esp32BannerIn{from{transform:translateY(-100%)}to{transform:translateY(0)}}';
  ...
  processSyncData(data) {
    ...
    let transitMs = 0;
    if (data.serverMs !== undefined && this.lastServerMs !== undefined) {
      transitMs = Math.max(0, receiveTime - this.lastReceiveTime - (data.serverMs - this.lastServerMs));
    }
    this.lastServerMs = data.serverMs;
    this.lastReceiveTime = receiveTime;

    // TTS Remoto
    if (data.speak !== undefined && data.speak !== '') {
      if (typeof _speakEvent === 'function') _speakEvent(data.speak);
      else if (typeof speakEventPhrase === 'function') speakEventPhrase(data.speak);
    }
    ...
    if (data.voiceVolume !== undefined) {
      let vVol = parseInt(data.voiceVolume);
      if (vVol !== window.currentVoiceVolume) {
        window.currentVoiceVolume = vVol;
        let slider = document.getElementById('voiceVolumeSlider');
        if (slider && slider.value != vVol) {
          slider.value = vVol;
          let disp = document.getElementById('voiceVolumeDisplay');
          if (disp) disp.innerText = vVol + '%';
        }
      }
    }
    ...
    if (data.maintenance !== undefined) {
      const newMaint = !!data.maintenance;
      if (newMaint !== isMaintenanceActive) {
        isMaintenanceActive = newMaint;
        updateMaintenanceUI();
        if (typeof syncMaintenanceButtonUI === 'function') syncMaintenanceButtonUI();
      }
    }

    let showNum = parseInt(data.show);
    let elapsed = (parseInt(data.elapsed) || 0) + Math.round(transitMs);
    
    // Gestione unificata dello Show State (Single Source of Truth)
    if (showNum > 0) {
      if (this.currentShow !== showNum) {
        console.log(`[SyncManager] Innesco show ${showNum} dal server, elapsed=${elapsed}ms`);
      }
      this.currentShow = showNum;
      
      // Controllo Show: deciderà se fare play o solo correggere il drift
      executeShow(showNum, elapsed);
    } else {
      if (this.currentShow > 0) {
        console.log('[SyncManager] Stop show rilevato da server');
        stopAllShowsLocal();
      }
      this.currentShow = 0;
    }
...
function chiudiAutoScreen() {
  ...
  if (window.speechSynthesis) {
   ...
   let homeVoiceVolume = 100;        // Volume voce assistente AI
   ...
   function updateClock() {
    ...
    function pollAutoSettings() {
      fetch('/get-auto-settings').then(r => r.json()).then(data => {
        ...
        if (data.voiceVolume !== undefined) {
            homeVoiceVolume = data.voiceVolume;
            window.currentVoiceVolume = homeVoiceVolume;
        }
   ...
   setInterval(updateClock, 1000);
   // Il SyncManager gestisce ora il polling di pollAutoSettings, l'health monitor e i banner offline.
...
window.checkMaintPassword = function() {
  ...
  if (pass === 'lori123' || pass === 'admin') {
    fetch('/set-maintenance?enable=0').then(() => {
      isMaintenanceActive = false;
      lastMaintenanceSpokenState = false;
      updateMaintenanceUI();
      speakPhrase(maintenanceEndPhrases);
    });
  } else {
...
function requestAdminAccess() {
  ...
  if (!modal) {
    ...
    window.checkAdminPass = function() {
      ...
      if (val === "lori123") {
        window.location.href = "control-area.html";
      } else {
...
function showScreen(screen) {
  ...
  if (screen === 'REGIA') {
    ...
    // Assicuriamoci che l'area admin sia visibile o nascosta a seconda del ruolo
    const adminSec = document.getElementById('adminSystemSection');
    if (adminSec) adminSec.style.display = (window.userRole === 'admin') ? '' : 'none';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (isApproved) {
    ...
    if (params.get('fast') === '1') {
      showScreen('REGIA');
      // Ripulisci l'URL senza ricaricare la pagina
